...
public class PrimaryController implements Controller {
    @FXML public VBox libView;
    @FXML public StackPane libStackPane;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (this.searchModel == null) {
            ...
            this.searchResults.addListener(
    ...
    private void libModuleCreate(Media media) { // should find a better way to solve this.
        ...
        if (media instanceof Book) {
            ...
            if (authorList.size() < 4) {
                authors =
                        authorList.stream()
                                .map(author -> author.getGivenName() + " " + author.getFamilyName())
                                .collect(Collectors.joining(", "));
            } else {
                authors =
                        authorList.subList(0, 3).stream()
                                .map(author -> author.getGivenName() + " " + author.getFamilyName())
                                .collect(Collectors.joining(", "));
            }
            ...
            borrowButton.setOnAction(
                    actionEvent -> {
                        ...
                        if (!(((Book) media).getPublisher() == null)) {
                            ...
                            mediaInformation
                                    .getChildren()
                                    .add(new Label(((Book) media).getPublisher()));
                        }
                        // ISBN
