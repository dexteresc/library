...
public class AccountManager {
    ...
    // Statements
    private static final String SELECT_CUSTOMER_BY_ID_STATEMENT =
            "SELECT *, (SELECT COUNT(*) FROM loan WHERE loan.customer_id = account.id AND returned_at IS NULL) AS active_loan_count FROM account INNER JOIN customer ON customer.account_id = account.id INNER JOIN customer_type ON customer_type.id = customer.customer_type_id WHERE account.id = ? LIMIT 1";
    private static final String SELECT_STAFF_BY_ID_STATEMENT =
            "SELECT * FROM account INNER JOIN staff ON staff.account_id = account.id WHERE account.id = ? LIMIT 1";
    private static final String UPDATE_ACCOUNT_PASSWORD_STATEMENT =
            "UPDATE account SET password_hash = ? WHERE id = ? LIMIT 1";
    private static final String UPDATE_ACCOUNT_STATEMENT =
            "UPDATE account SET given_name = ?, family_name = ?, email = ?, phone = ? WHERE id = ? LIMIT 1";
    private static final String CREATE_ACCOUNT_STATEMENT =
            "INSERT INTO account (given_name, family_name, email, phone, password_hash) VALUES (?, ?, ?, ?, ?)";
    private static final String CREATE_CUSTOMER_STATEMENT =
            "INSERT INTO customer (account_id, customer_type_id) VALUES (?, ?)";
    private static final String DELETE_ACCOUNT_BY_ID_STATEMENT =
            "DELETE FROM account WHERE id = ? LIMIT 1";
    private static final String DELETE_ACCOUNT_BY_EMAIL_STATEMENT =
            "DELETE FROM account WHERE email = ? LIMIT 1";
     ...
     * @throws Exception if the account violates unique constraints, or if a general database error
     *     occurs.
     * @implNote This is a blocking operation.
    ...
    public void createCustomerAccount(Customer customer, String password) throws Exception {
        ...
        this.database
                .insert(CREATE_CUSTOMER_STATEMENT)
     ...
     * @throws Exception if the account instance violates database constraints, or if a general
    ...
    public void deleteAccount(Account account) throws Exception {
        ...
        database.delete(DELETE_ACCOUNT_BY_ID_STATEMENT).configure(account.getId()).execute();
    }
}
