...
public class AuthenticationModel {
    ...
    private static final String LOGIN_STATEMENT =
            "SELECT id, password_hash, EXISTS(SELECT * FROM customer WHERE account_id = id) AS is_customer, EXISTS(SELECT * FROM staff WHERE account_id = id) AS is_staff FROM account WHERE email = ? LIMIT 1";
        ...
        return database.select(LOGIN_STATEMENT, Account.class)
                ...
                .fetch(
                        resultSet -> {
                            ...
                            BCrypt.Result verificationResult =
                                    verifier.verify(
                                            password.toCharArray(),
                                            resultSet.getString("password_hash"));

                            if (!verificationResult.verified) {
                                // The password was not a match, or there was an error related to
                                // hash formatting.
                                // Might be worth checking if verification result returns an invalid
                            ...
                            if (resultSet.getBoolean("is_staff")) {
                                ...
                                // Account is neither staff nor customer. This scenario should be
                                // avoided.
                                throw new Exception();
                            }
                        });
