...
public class MediaItemManager extends MediaManager {
    ...
    private static final String SELECT_MEDIA_ITEMS_BY_MEDIA_ID_STATEMENT =
            "SELECT *, (SELECT COUNT(*) FROM loan WHERE loan.media_item_id = media_item.id AND returned_at IS NULL) AS currently_on_loan FROM media_item INNER JOIN media_type ON media_item.media_type_id = media_type.id WHERE media_id = ?";
    private static final String SELECT_AVAILABLE_MEDIA_ITEMS_BY_MEDIA_ID_STATEMENT =
            "SELECT *, (SELECT 0) AS currently_on_loan FROM media_item INNER JOIN media_type ON media_item.media_type_id = media_type.id WHERE media_id = ? AND status IS NULL AND media_type.loan_period > 0 AND media_item.id NOT IN (SELECT media_item_id FROM loan WHERE media_item_id = media_item.media_id)";
    private static final String SELECT_FIRST_AVAILABLE_MEDIA_ITEMS_BY_MEDIA_ID_STATEMENT =
            "SELECT *, (SELECT 0) AS currently_on_loan FROM media_item INNER JOIN media_type ON media_item.media_type_id = media_type.id WHERE media_id = ? AND status IS NULL AND media_type.loan_period > 0 AND media_item.id NOT IN (SELECT media_item_id FROM loan WHERE media_item_id = media_item.media_id) LIMIT 1";
    private static final String CREATE_MEDIA_ITEM_STATEMENT =
            "INSERT INTO media_item (media_id, media_type_id, status) VALUES (?, ?, ?)";
    private static final String UPDATE_MEDIA_ITEM_STATEMENT =
            "UPDATE media_item SET media_type_id = ?, status = ? WHERE id = ?";
    private static final String DELETE_MEDIA_ITEM_STATEMENT = "DELETE FROM media_item WHERE id = ?";

    private static final String SELECT_ALL_MEDIA_TYPES_STATEMENT =
            "SELECT id AS media_type_id, type_name, loan_period FROM media_type";
    private static final String CREATE_MEDIA_TYPE_STATEMENT =
            "INSERT INTO media_type (type_name, loan_period) VALUES (?, ?)";
    private static final String UPDATE_MEDIA_TYPE_STATEMENT =
            "UPDATE media_type SET type_name = ?, loan_period = ? WHERE id = ?";
                ...
                .fetchAll(
                        resultSet ->
                                new MediaItem(
                                        ...
                                        media,
                                        new MediaType(resultSet),
                                        resultSet.getInt("currently_on_loan") == 1,
                                        MediaItem.Status.resolve(resultSet.getString("status"))));
    }
    ...
    public MediaItem getFirstAvailableMediaItem(Media media) throws Exception {
        logger.info(
                "Getting first available media item for media with id " + media.getId() + "...");
        return database.select(
                        SELECT_FIRST_AVAILABLE_MEDIA_ITEMS_BY_MEDIA_ID_STATEMENT, MediaItem.class)
                .configure(media.getId())
                .fetch(
                        resultSet ->
                                new MediaItem(
    ...
    public void updateMediaItem(MediaItem mediaItem) throws Exception {
        ...
        database.update(UPDATE_MEDIA_ITEM_STATEMENT)
                .configure(
                        mediaItem.getMediaType().getId(),
                        mediaItem.getStatus().getRawValue(),
                        mediaItem.getId())
                .execute();
    }
    ...
    public void updateMediaItems(Media media, List<MediaItem> mediaItems) throws Exception {
        ...
        List<MediaItem> mediaItemsToAdd =
                mediaItems.stream()
                        .filter(mediaItem -> !mediaItemsBeforeUpdate.contains(mediaItem))
                        .collect(Collectors.toList());

        // Filter media items to update
        List<MediaItem> mediaItemsToUpdate =
                mediaItems.stream()
                        .filter(mediaItemsBeforeUpdate::contains)
                        .collect(Collectors.toList());

        // Filter media items to delete
        List<MediaItem> mediaItemsToDelete =
                mediaItemsBeforeUpdate.stream()
                        .filter(mediaItem -> !mediaItems.contains(mediaItem))
                        .collect(Collectors.toList());
    ...
    public void createMediaItem(MediaItem mediaItem) throws Exception {
        ...
        Long id =
                database.insert(CREATE_MEDIA_ITEM_STATEMENT)
                        .configure(
                                mediaItem.getMedia().getId(),
                                mediaItem.getMediaType().getId(),
                                mediaItem.getStatus().getRawValue())
                        .executeQuery();
        mediaItem.setId(id);
    ...
    public void deleteMediaItem(MediaItem mediaItem) throws Exception {
        ...
        database.delete(DELETE_MEDIA_ITEM_STATEMENT).configure(mediaItem.getId()).execute();
    }
    ...
    public void createMediaType(MediaType mediaType) throws Exception {
        ...
        Long id =
                database.insert(CREATE_MEDIA_TYPE_STATEMENT)
    ...
    public void deleteMediaType(MediaType mediaType) throws Exception {
        ...
        database.delete(DELETE_MEDIA_TYPE_STATEMENT).configure(mediaType.getId()).executeQuery();
    }
}
